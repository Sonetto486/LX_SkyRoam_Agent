    ...
    try:
        ...
        async with engine.begin() as conn:
            ...
            if not table_exists.scalar():
                logger.debug("users 表不存在，跳过画像字段检查")
                return

            # 逐个字段检查并补齐，避免一次性 ALTER 失败导致全部中断
            column_sql = {
                "favorite_locations": "ALTER TABLE users ADD COLUMN favorite_locations JSONB",
                "highlighted_locations": "ALTER TABLE users ADD COLUMN highlighted_locations JSONB",
                "special_focus": "ALTER TABLE users ADD COLUMN special_focus JSONB",
                "photo_mood": "ALTER TABLE users ADD COLUMN photo_mood TEXT",
            }
            ...
            for column_name, alter_sql in column_sql.items():
                ...
                if column_exists.scalar():
                    logger.debug(f"✓ users.{column_name} 字段已存在")
                    continue

                logger.info(f"正在为 users 表添加 {column_name} 字段...")
                await conn.execute(text(alter_sql))
                logger.info(f"✅ users.{column_name} 字段添加成功")
