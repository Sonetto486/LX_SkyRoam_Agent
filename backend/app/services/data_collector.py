        ...
        try:
            ...
            if cached_data and not must_visit_attractions:
                ...
                if cached_data and len(cached_data) > 0:
                    ...
                    source = first_item.get("source", "未知")
                    logger.info(f"缓存数据来源: {source}，共 {len(cached_data)} 条")
                return cached_data
            ...
            # ========== 优先从 POI 向量数据库检索景点 ==========
            logger.info(f"========== 开始POI向量检索，目的地: {destination} ==========")
            try:
                from app.services.poi_retriever import get_poi_retriever
                poi_retriever = get_poi_retriever()

                logger.info(f"POI检索器初始化成功，开始向量检索...")

                # 1. 向量检索景点
                poi_attractions = poi_retriever.retrieve_by_destination(
                    destination=destination,
                    top_k=desired_min_attractions * 2
                )

                logger.info(f"向量检索返回 {len(poi_attractions) if poi_attractions else 0} 个景点")

                if poi_attractions:
                    logger.info(f"POI向量库检索到 {len(poi_attractions)} 个景点")

                    # 打印前3个景点用于调试
                    for i, attr in enumerate(poi_attractions[:3]):
                        logger.info(f"  POI景点{i+1}: {attr.get('name')} ({attr.get('city')})")

                    # 2. 高德 API 补充实时信息
                    logger.info(f"开始调用高德API补充实时信息...")
                    enriched_attractions = await poi_retriever.retrieve_with_amap_enrichment(
                        destination=destination,
                        top_k=desired_min_attractions
                    )
                    ...
                    logger.warning(f"POI向量检索返回空结果，回退到高德地图")

            except Exception as e:
                logger.warning(f"POI检索失败，回退到高德地图: {e}")
                import traceback
                logger.warning(f"POI检索异常堆栈: {traceback.format_exc()}")
